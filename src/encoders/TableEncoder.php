...
Class TableEncoder implements EncoderInterface
{
    ...
    public function out($obj)
    {
        ...
        if( !empty($obj->data) ){ 
            ...
            $path = preg_replace('/with=[^&]*/','',$obj->path);$obj->path = str_replace('?&','?',$obj->path);$obj->path = str_replace('&&','&',$obj->path);
            ...
            echo '<body>';
            $csv_path = str_replace('otable','ocsv',$obj->path);
            $tsv_path = str_replace('otable','otsv',$obj->path);
            $json_path = str_replace(['?otable','&otable'],'',$obj->path);
